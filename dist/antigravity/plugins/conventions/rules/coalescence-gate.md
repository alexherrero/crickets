...
**This is mostly a session discipline, not new machinery** — a coalescence check spans boards, the vault, release bodies, and the wiki, surfaces a repo-local CI job can't see end to end. The discipline lives in the `/release` phase spec (`development-lifecycle`'s `release.md`, constraint 9): when a release closes an arc, work this checklist before tagging. **A narrow slice of three items (1, 3, 5) now has a deterministic backstop** — `ship-release`'s own mechanical-workflow precondition runs [`scripts/coalescence_checks.py`](../scripts/coalescence_checks.py) (a cheap file-read or `gh` API assertion per check) and aborts the cut with a specific message if any of the three fail, on every release, not only an arc-closing one: the release's own tag has a Completed-Features row, no `PLAN.archive.*` file sits flat outside `archive/`, and — when a board item id is given — that item's row is closed. That is real, but partial: item 1's *every feature the arc shipped* and item 3's *every board this arc touched, plain-English pass included* still need a human read at arc-close; the script only ever checks the one release/one item slice named above. Items 2, 4, 6, and 7 are untouched by this script and stay prose, checked only when a release closes an arc.

### The seven items

1. **Narrative rows appended.** The completed-features timeline (a repo's `Completed-Features.md`-equivalent reference page) gains one row per feature this arc shipped — plain English, one sentence, release tag, roadmap id. `ship-release`'s own "append the feature row(s)" step is the maintainer; confirm it ran for everything the arc shipped, not just the release being cut right now. **Machine-checked slice:** `coalescence_checks.py`'s `narrative-row` check blocks the cut if the release's own tag has no matching row at all.
...
- [`scripts/coalescence_checks.py`](../scripts/coalescence_checks.py) — the deterministic backstop for items 1, 3, and 5's narrowest slices, run from `ship-release`'s mechanical-workflow precondition 6 on every release.
