...
You are running **`/open`** (also invocable as `/orient` — the two commands are one implementation, two entry points). It resolves the operator's "open the file for X" intent to a project/idea/context, confirms the match, and renders a short read-only orientation. **It never resumes work, marks a task, or activates a plan.**
