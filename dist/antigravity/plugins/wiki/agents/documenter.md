...
- **rc 2** — vault reachable but this project isn't registered. The bundle still carries operator-global conventions from the always-load tier (`<vault>/standards/`, or the retired `_always-load/` pen on a vault from before the memory-root trims); honor those and fall back to repo-local context for project specifics.
...
3. **Resolve the genre overlay** — glob the vault's global voice store for the latest-dated `*docs-prose-style*.md` file (the wiki-page genre overlay; recent wins per the standard wiki-style precedence) and pass its **absolute path** via `--overlay`. Find the store by probing newest layout first — `<vault>/standards/voice/` (beside the memory root since the memory-root trims, agentm-vault plan 05), then the retired `_global/wiki-style/` store on each project-space rung: the vault-root `Projects/_global/wiki-style/` (filing-v2 2b), `<memory-root>/desk/projects/_global/wiki-style/`, `<memory-root>/projects/_global/wiki-style/` — and take the first that exists: the store has moved between generations, and a vault mid-migration still holds it on an older rung. That is the chain [`scripts/vault_layout.py`](../skills/diataxis-author/scripts/vault_layout.py) walks; a pinned literal reads as an empty store, which is silent. `prose_pass.py`'s own default overlay is `design-doc-prose`, wrong for a wiki page — always override it here.
