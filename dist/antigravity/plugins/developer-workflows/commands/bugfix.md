...
Run **`/review`** on the fix (non-negotiable for bugs). Confirm: the regression test is committed and **actually exercises the root cause** (not just the symptom); the original reproduction steps from `## Report` now produce the expected behavior. Dispatch the documenter (graceful-skip; **lightweight** — most bugfixes get `NO CHANGES`; it updates a Known-Issues page or adds an ADR **only** if the fix reveals a durable gotcha — over-documentation is drift too).
