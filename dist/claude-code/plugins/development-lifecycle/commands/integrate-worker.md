...
7. **Integration is serialized — one at a time (LC-1/LC-5).** The helper holds a per-repo advisory lock (`.git/integrate.lock`, separate from spawn's worktree-add lock) across the whole merge → gate critical section, so a second concurrent `/integrate-worker` **blocks** rather than racing on the shared integration branch + version registry (single-writer integration, development-lifecycle design). Build is N-wide; the integrator is the single writer. Do not work around the lock to land two workers at once.
