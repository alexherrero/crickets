...
**Status as of this rule's authoring (CONS-8, 2026-07-11): documented, not yet wired as a live call site.** `fanout_cost_gate.py` is built and tested but has no caller in any coordinator flow today — this rule is its first named convention-level use, not its first wired integration. Wiring it in as an actual pre-flight check (a call site in the coordinator role / dispatch tooling that runs before a fan-out and honors a block) is real integration work — deciding where a "coordinator" session's tooling lives, what counts as the budget-share config, and how a block surfaces — and is **out of scope for this rule's own authoring pass**. It is named here as a small, explicit follow-up: the next multi-session coordinator dispatch that wants the gate live should wire this call site rather than re-discover the gate exists.
