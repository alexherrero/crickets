...
## Host support — Claude-only

`evidence-tracker` is declared `supported_hosts: [claude-code]` because its core value is **enforcement** — blocking a `[ ]`→`[x]` flip (exit 2) when evidence is unmet. That requires the host to honor a hook's veto, which only Claude Code does. Antigravity plugin hooks are observe/side-effect-only (the host ignores hook exit codes), so an evidence gate there could record reads but never *enforce* — a useless half-feature. Rather than ship an observe-only no-op, the hook is Claude-only (the same call as the `harness-context` SessionStart precedent). **Re-audit if Antigravity ships hook-veto.**
