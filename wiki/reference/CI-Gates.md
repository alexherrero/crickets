...
| `vault-content tests` | `test_voice_kernel.py`, asserting facts about a real MemoryVault: the voice kernel is locatable and small, the three heavy genre files stayed demoted out of the always-load tier and resolvable on demand, and the kernel still reaches at least one live consumer. Runs **without** the `AGENTM_INSTALL_PREFIX` / `MEMORY_VAULT_PATH` isolation the `unit tests` gate uses, because that scratch prefix makes these skip themselves into silence. Skips cleanly when no vault resolves, so it is a no-op in CI and on any machine without one. |
