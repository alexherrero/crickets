...
| `no-harness-paths` | `check-no-harness-paths.py` — no plugin names the vault's retired harness directory, tests included: a project's homes are its own `tasks/`, `designs/` and `desk/`, and a plugin asks agentm for them. A line may keep the word with a `harness-deprecation:` marker. The script's `EXEMPT` list names the files the in-flight part 15 landings still clear, and a listed file with no hits left fails the gate, so the list only shrinks. `--inventory` prints every hit by plugin. agentm runs the same gate under the same name |
...
| `vault-content tests` | `test_voice_kernel.py`, asserting facts about a real MemoryVault: the voice kernel is locatable and small, the three heavy genre files stayed demoted out of the always-load tier and resolvable on demand, and the kernel still reaches at least one live consumer. Runs **without** the `AGENTM_INSTALL_PREFIX` / `MEMORY_ROOT` (and its deprecated alias `MEMORY_VAULT_PATH`) isolation the `unit tests` gate uses, because that scratch prefix makes these skip themselves into silence. Skips cleanly when no vault resolves, so it is a no-op in CI and on any machine without one. `test_voice_kernel_layouts.py`, part of `unit tests`, runs the suite against scratch vaults on every layout the kernel has lived on, so CI still catches the suite drifting from `prose_pass`. |
