...
5. **Shows `✘ failed to load` after an upgrade** → the plugin was **renamed or removed** (e.g. `wiki-maintenance` → `wiki`, the AG Wave A rename wave, v3.24.0 — reversing an earlier `wiki` → `wiki-maintenance` rename from v3.2.0 that this same plugin also went through). The old install points at a name the marketplace no longer offers. Uninstall the old name and install the new one:
   ```bash
