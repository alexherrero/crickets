...
| [`adversarial-reviewer-cross`](https://github.com/alexherrero/crickets/blob/main/src/code-review/agents/adversarial-reviewer-cross.md) | sub-agent | The same, cross-model via Gemini; degrades visibly — a `CROSS-REVIEW-DEGRADED: <reason>` line, never a silent same-model swap, whenever `cross-review.sh` exits non-zero: `agy` missing or failing, its print timeout running out, material over the size ceiling, or a reply that breaks the output contract twice. |
