...
dev-setup (optional; operator-personal; not surfaced in agentm/crickets docs)
└── Adds shell-env wiring if present; agentm install is self-sufficient otherwise.
```
...
Installs device-wide. Detects dev-setup presence; defers to it if present (operator-personal shell-env layer); otherwise writes directly to `~/.zshrc` (or detected shell config). Cross-platform parity with `install.ps1`.
...
Pre-pipe notes in each repo's README make the asymmetric dependency clear without requiring operator's personal `dev-setup` tool.
