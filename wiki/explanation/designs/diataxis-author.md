...
4. **The dogfood surface is rich** — the operator maintains three Diátaxis-shaped wikis already (agentm, crickets, dev-setup) plus the just-shipped MemoryVault parent design. Plenty of real authoring to validate the skill against.
...
5. **AgentMemory read-only (no write-back of operator decisions).** Rejected per Q3 answer: the skill becomes consistent across the operator's three Diátaxis-shaped wikis (agentm + crickets + dev-setup) only if learned conventions write back to `_always-load/`. Read-only would force operator to manually `/memory save` every judgment call.
...
2. **Convention drift across operator's three Diátaxis wikis** (agentm + crickets + dev-setup). If skill learns a convention in one repo + applies in another but conventions differ subtly, operator sees surprise. **Mitigation**: per-repo `.diataxis-conventions.md` override file (per Migrations §3). Always-load conventions are global defaults; per-repo file overrides for that one project.
