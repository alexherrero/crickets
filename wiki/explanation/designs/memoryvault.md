...
**External**:
- *(historical: v1 spec listed Anthropic API for embeddings; v0.9.2 dropped this — see [ADR 0001's 2026-05-20 amendment](../decisions/0001-agent-toolkit-purpose.md#amendment-2026-05-20). `sentence-transformers` below is now the only embedding path.)*
...
7. (L) Recall engine — sqlite-vec + grep+frontmatter merge + local embedding integration (BGE-large via sentence-transformers; see [ADR 0001 amendment](../decisions/0001-agent-toolkit-purpose.md#amendment-2026-05-20) for the v0.9.2 local-only refactor).
...
5. (M) Release pair `agent-toolkit v0.9.2` + harness if needed.
...
3. **#7b release**: `agent-toolkit v0.9.2`. Lands transcript reflection + personal-skills indexer + internet skill-discovery.
