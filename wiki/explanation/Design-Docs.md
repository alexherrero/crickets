...
| Requires (hard) | [Development Lifecycle](Development-Lifecycle) | The `/design` command delegates to development-lifecycle's `/design` implementation; without it the command has nothing to delegate to. |
...
- You do not run the harness's plan-driven workflow — the `/design` pipeline assumes `development-lifecycle` is installed to do the real work, so on its own the command is inert.
...
|---|---|---|
| [`/design`](https://github.com/alexherrero/crickets/blob/main/src/design/commands/design.md) | command | Author → translate → sequence a design doc into named plans; delegates to development-lifecycle. |
| [`adr`](https://github.com/alexherrero/crickets/blob/main/src/design/skills/adr/SKILL.md) | skill | Decision-record discipline — when to write an ADR and the five-section shape with re-audit triggers. |

### Configuration

No configuration — the plugin works out of the box. The `/design` command reads its workflow settings from `development-lifecycle`, and the `adr` skill draws its canonical shape from the `adr-shape` entry in `~/.claude/CLAUDE.md`; neither adds a config key of its own.
...
- [Development Lifecycle](Development-Lifecycle) — owns the `/design` and `/document-decision` implementations this plugin builds on.
