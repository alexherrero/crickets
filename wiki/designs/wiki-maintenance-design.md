...
*Folded decision history (AG Phase-2 C4 — records retired into this wiki design; git holds the full ADR text).*

**2026-06-21 (C4 fold) — ADRs 0008 + 0018 retired into this design (AG Phase 2).** The agentm/crickets ADR model was retired (AG design-doc §5); ADR 0008 (the `diataxis-author` skill) + ADR 0018 (per-folder sidebars) folded here and deleted via `migrate-adr.py` (inbound links repointed, index/sidebars pruned). This design was stamped as the `crickets/wiki` area owner (`governs: src/wiki-maintenance/**`); the provisioning + section-taxonomy designs are children in the same area. *Why not keep them as ADRs:* the append-only model forces a chain-read; the living body is the single source. *Re-audit trigger:* the Diátaxis four-mode taxonomy ceasing to be canonical, or GitHub Wiki changing how it renders the nearest `_Sidebar.md`.

**2026-05-22 — the `diataxis-author` skill (was ADR 0008).** Shipped `diataxis-author` as a single crickets skill with five sub-commands (`author`/`check`/`repair`/`migrate`/`classify`), mirroring `/memory`'s shape: deterministic Python pipelines + one read-only `diataxis-evaluator` sub-agent (zero filesystem write scope) for ambiguous mode-classification; it **subsumed** the `migrate-to-diataxis` predecessor and repurposed the harness `documenter` as its mechanical-write worker. AgentMemory integration is read + operator-confirmed write (per-repo `.diataxis-conventions.md` overrides + global vault entries). *Amended 2026-06-06 (kept):* the write-back **split by lesson kind** — voice/style lessons relocated to the on-demand `<vault>/projects/_global/wiki-style/` store (composed at draft time by `style_resolver.py`, never auto-loaded; agentm ADR 0010 vault-taxonomy); only structural-naming conventions remain in `_always-load`. *Why not author-only / keep the predecessor / read-only memory:* the maintenance gap needs the full lifecycle; two Diátaxis skills is mental-model burden; read-only forces an operator-tax on every judgment call. *Note:* the skill itself moved to agentm in v2.0.0 (V4 #36); this records the crickets-era decision. *Re-audit trigger:* a model bump changing how agents consume mode-separated docs, or the four-mode taxonomy being deprecated.

**2026-06-08 — per-folder sidebars: intent-grouped wiki folders with nearest-`_Sidebar.md` nav (was ADR 0018).** Organized the wiki into intent-group folders, each carrying its own `_Sidebar.md` that GitHub Wiki renders by walking up to the nearest one (verified empirically) — contextual collapse/expand nav with no platform migration; a `<!-- mode: X -->` hint preserves Diátaxis mode-enforcement where an intent folder mixes modes; `check-wiki` rule-j checks the union of all sidebars (≤2-click reachability), not "root lists everything." *Why not type-folders / a dynamic JS sidebar / a content heuristic:* type-folders contradict the intent ordering; a dynamic sidebar means leaving GitHub Wiki; a content heuristic is fragile. *Superseded-in-spirit:* the intent-folder **section structure** was later reframed by the seven-section taxonomy (see [wiki-section-taxonomy](wiki-section-taxonomy)); the nearest-`_Sidebar.md` **mechanism** persists. *Re-audit trigger:* GitHub changing wiki rendering / migrating off Gollum, or `wiki-sync` flattening the tree.
