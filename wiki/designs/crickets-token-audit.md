...
- **Ambient capture *(designed — staged now, a Wave-D pull-forward)*** — on session-stop a hook auto-runs the analyzer and **logs the per-model cost breakdown into memory** as a durable `session-cost` record (a new memory kind). This half doesn't wait on the dreaming pass to exist — it only needs a place to write, and it independently feeds the pre-flight fan-out cost gate below (its per-agent-cost estimate) as soon as a few records accumulate. **Flagged, not decided here:** this pulls forward implementation the roadmap had scoped for a later wave; the roadmap session should confirm the pull-forward rather than treat it as settled by this design. **`[PENDING-IMPL]`**.
- **Periodic review *(designed, still gated)*** — the memory engine's **dreaming pass** (its periodic reflection / consolidation cycle) reviews the accumulated `session-cost` records to surface an efficiency *trend* — flagging when token use creeps up over time. This turns token-audit from a point-in-time lookup into a longitudinal signal on the **`efficient`** opinion, but it cannot build before the dreaming pass itself does. **`[PENDING-IMPL]`.**
...
- **(designed) couples to the agentm memory engine** — the auto-on-stop capture writes a `session-cost` memory record (staged now, a Wave-D pull-forward); the dreaming-pass review reads it later, unchanged in its own gating. The *delivered* capability stays standalone.
...
- **All shipping primitives delivered** — `/token-audit`, `analyzer.py`, `pricing.py`, `status_line_meter.py`; the fold + rename are mechanical. The automation layer (budget readout, fan-out gate, `/handoff-pack`, routing-conformance, the versioned routing table) and the ambient capture + periodic review are the designed additions (above).
...
- **The capture/review split is a coupled bet on timing, and the capture side is a wave pull-forward.** The Stop-hook capture ships independent of the dreaming pass — useful on its own, since the fan-out gate needs it — but building it "now" pulls forward implementation the roadmap had scoped later; that call belongs to the roadmap session, flagged here rather than decided.
- **Re-audit triggers:** roadmap session either confirms or reschedules the Wave-D ambient-capture pull-forward; stand up the content-refresh capability to automate the `pricing.py` + routing-table re-pin (manual re-pin until then); build the automation layer (fan-out gate, budget readout, `/handoff-pack`, routing-conformance) via `PLAN-efficiency-automation`; wire the dreaming-pass review when the memory engine's relevant wave ships; migrate the fan-out gate + conformance report from hardwired to `opinion_request('efficient')` when the Wave-B opinion registry lands; execute the `status-line-meter` fold + the `tokens` rename at v6.0; add Gemini/Antigravity rows to the routing table when that side of the portfolio needs its own consumption path; replace the local cost estimate with ground truth if a host ever exposes real per-session quota telemetry.
