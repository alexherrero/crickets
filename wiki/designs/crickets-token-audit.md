...
- **Ambient capture *(designed — staged now, a Wave-D pull-forward)*** — on session-stop a hook auto-runs the analyzer and **logs the per-model cost breakdown into memory** as a durable `session-cost` record. **The place to write now exists:** `session-cost` is a reserved `kind` value in agentm's V6-11 metadata table, shipped 2026-07-06 (see agentm's [memory index](https://github.com/alexherrero/agentm/wiki/agentm-memory-index) design). The capture hook itself is tracked separately (`PLAN-session-cost-capture`, confirmed build-now, capture-half only) — this half doesn't wait on the dreaming pass to exist, and it independently feeds the pre-flight fan-out cost gate below (its per-agent-cost estimate) as soon as a few records accumulate. **`[PENDING-IMPL]`** until that plan lands.
