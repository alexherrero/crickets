...
- **Ambient capture — retargeted onto the observability ledger, 2026-07-07** (`PLAN-observability-ledger`, Autonomy arc AA3) — on session-stop, the `session-cost-capture` Stop hook (`src/tokens/hooks/session-cost-capture/`) runs `session_cost_writer.py` over the closing transcript and appends one `session-cost` telemetry event per model to a device-local, monthly-rotated JSONL event log (`~/.agentm/telemetry/events-YYYYMM.jsonl`, `$AGENTM_TELEMETRY_DIR`-overridable), tagged with `{plan, task, arc, grade}` attribution stamped from the worktree-local `.harness/active-plan` marker. Graceful no-op with an unwritable telemetry path — never blocks session close. `session_cost_reader.py` reads the log back for the pre-flight fan-out cost gate below, which now runs against real data instead of its fallback profile. **No vault write survives this capability** — the prior `kind: session-cost` markdown write via agentm's `save_entry()` bridge is retired, not duplicated; see the Autonomy arc's [observability ledger and console](https://github.com/alexherrero/agentm/wiki/agentm-autonomy) design this move implements.
...
- **no longer couples to the agentm memory engine** — the auto-on-stop capture used to write a `kind: session-cost` memory record; that vault write is retired (**delivered** 2026-07-07, see the ambient-capture bullet above). The dreaming-pass review remains designed, unchanged in its own gating on the dreaming pass.
- **feeds the agentm-side [observability ledger](https://github.com/alexherrero/agentm/wiki/agentm-autonomy) — delivered 2026-07-07** — this capability's writer/reader now target the device-local JSONL event log directly; agentm's `PLAN-observability-ledger` (agentm half) hosts the runner job that folds the raw log into the ledger's SQLite rollup — the fan-out gate here still reads the raw event log, not the rollup (the rollup is a downstream aggregate, not this capability's read path).
...
**2026-07-07 — the ambient capture retarget is delivered (AA3, `PLAN-observability-ledger` crickets half).** Flips the prior "superseded by design, not yet migrated" note to as-built: `session_cost_writer.py` no longer bridges to agentm's `save_entry()` / the vault at all — it appends `session-cost` telemetry events (`{model, tokens_by_kind, cost_usd, tags}`) to the device-local, monthly-rotated JSONL event log via the new `event_log.py` module, with attribution tags stamped from the worktree-local `.harness/active-plan` marker. `session_cost_reader.py` reads the same log back for the fan-out gate. No vault call site remains in `src/tokens/` (grep-confirmed, task 4's own verification). agentm's `PLAN-observability-ledger` (agentm half) picks up from here: the runner-hosted aggregator that folds this log into the ledger's SQLite rollup, plus the doc-pointer flip on agentm's own `agentm-memory-index.md`. *Re-audit trigger:* none outstanding for this capability's own half — the remaining ledger work (rollup, console, digests) lives entirely downstream in agentm's designs.
