...
3. **The opinions** — design **consumes `how-we-engineer`** (the sizing ladder, the templates, and the section conventions *are* how we engineer designs — `how-we-engineer` itself `implements: crickets/development-lifecycle`, not design) and **consumes `good`** (what a sound design reads like — the adversarial-read bar). One-way: design renders the standard into a document; it does not author the opinion.
4. **The voice** — the operator's prose voice (`docs-prose-style`, an agentm **opinion**: subjective and learned, *not* a convention) is folded in so the design reads in one voice. That voice is second-person and plain: no peacock or AI-tell adjectives, no prior-art name-drops, no "this-not-that" antithesis, labels de-performed, plain names over insider metaphors. Since 2026-07-16 the voice is also *enforced at read-time*: the **`prose-pass` skill** (Gemini simplifies via `scripts/prose_pass.py` with the voice pack inlined verbatim, Claude fact-checks and applies) runs over the full document at author Step 5 and review-pass entry — the operator's fixed `design-doc-prose` overlay is the default `--overlay`. Request-by-name through the opinion registry stays `[PENDING-IMPL]`.
...
1. **Generation** — `sequence` opens the queued tasks directly: one `/plan`-shaped body per part, each in a numbered task with a `queued` tracker, mapping the parent's scope → Brief, verification → Goal, locked calls → Constraints, sibling parts → Out-of-scope, with `parent_design_doc` traceability — then hands off to the normal `/work → /review → /release` loop.
...
**Delivered:** the `/design` author → translate → sequence pipeline · `/spec` · `/interview-me` · `/document-decision` · `design_doc.py` · `design_sequence.py` · `stage_plan.py` · the full design-doc template · the `adr` skill · the grounding hooks · the **headless cross-model prose pass** (`prose-pass` skill + `prose_pass.py` — Gemini simplifies, Claude verifies; shipped 2026-07-16).

**To build (`[PENDING-IMPL]`):** the **architecture-rung scaffolding** (multi-system HLD + cross-system composition analysis + architecture-review — generalize from the AG HLD set); the **abbreviated-design template** packaged as a runnable `/design` template (an AG shape-spec today); the **finalize tooling** (auto-collapse the amendment log + the `[PENDING-IMPL]` stale-placeholder grep check — run by hand today, as in this set); **request-by-name** of the `how-we-engineer` / `good` / voice opinions (the Phase-3/4 registry); the **design-authoring self-improvement loop**; and the **interactive external-review handoff** (#5b — its headless sibling shipped as `prose-pass`; the transfer-context + resume session flow stays deferred).

### Opinions it consumes

design **consumes `how-we-engineer`** (layer 3 — the sizing ladder + the section conventions *are* how we engineer designs; `how-we-engineer` `implements: crickets/development-lifecycle`, so design is a cross-plugin consumer, not the implementer) and **consumes `good`** (the read-bar); the operator's **voice** is the third opinion it folds in (layer 4). The arrow is one-way — design renders the standards into a document, it does not author them. *(Both bindings interpolated at build time from a committed snapshot via the markdown-prose consumer grammar — `good` since PLAN-opinion-consumer-grammar task 2, `how-we-engineer` since PLAN-wave-d-personas task 2; request-by-name is separately deferred to the Phase-3/4 registry work — the [Opinions design](https://github.com/alexherrero/agentm/wiki/agentm-opinions-and-gates).)*

## Dependencies

- **requires [development-lifecycle](crickets-development-lifecycle.md)** — design sits at the head of the loop, above `/plan`; `sequence` hands the loop its queued tasks, and the grounding hooks bind `/plan` · `/review` · `/release` back to the design.
- **consumes `how-we-engineer` + `good`** ([agentm Opinions](https://github.com/alexherrero/agentm/wiki/agentm-opinions-and-gates)) — the sizing ladder *is* how-we-engineer; `good` is the read-bar. Both interpolated at build time from a committed snapshot (design does not implement either — `how-we-engineer` `implements: crickets/development-lifecycle`, `good` `implements: crickets/code-review`). It also folds in the operator's **voice** (an agentm opinion; always-load prose today, base⊕overlay request-by-name `[PENDING-IMPL]`).
...
- **Voice:** the `## Voice` section of `<vault>/standards/user-preferences.md` (the always-on essence + tells; a vault from before the memory-root trims keeps it as `voice-kernel.md` in the retired `_always-load/` pen) · `docs-prose-style` in `<vault>/standards/voice/` (the on-demand design/wiki prose refinement, demoted from always-load 2026-07-05 per PLAN-r3-voice-mechanism — see [wiki](crickets-wiki.md)'s 2026-07-05 amendment; the retired `_global/wiki-style/` store is the fallback) · `style_resolver.py` (the base⊕overlay seam, built for wiki)
...
**2026-09-22 — `/design sequence` opens queued tasks (design 0.11.0; agentm-vault part 15, crickets task 100).** `sequence` activated the first part as `PLAN-<doc-slug>-<part-slug>.md` and staged the rest in `queued-plans/`, a flat tier the projects migration dissolved. Every part is now a new numbered task, `tasks/NNN-<doc-slug>-<part-slug>/`, whose tracker opens at `queued` and names the design. `design_sequence.py check-names` refuses every name before anything is written: a name agentm places outside a task, one it names no tracker for, or one that already names a task. `place` then writes one part at a time, because agentm numbers a new task from the directories that exist. None is activated. *Why not keep activating the first part:* sequencing an arc is authoring. Starting work is the operator's word, and a task activates in place with `/plan --activate` or `/work --name`. *Why task names stay `<doc-slug>-<part-slug>`:* the migration named agentm-vault's parts that way, and the names are listed for the operator to confirm first. *Re-audit* if a project wants an arc's tasks started in order without a word per task.

**2026-09-22 — confidential designs, parts and specs live in the project's own homes (design 0.11.0; agentm-vault part 15, crickets task 100).** `design_doc.resolve_harness_root` took the parent of the resolved singleton `PLAN.md` as the place for confidential designs and parts. On a project that keeps its plans in tasks, the singleton call is exit 4, so `/design author` with a confidential design, and translate and sequence, could not run on agentm or crickets after the projects migration. `design_doc.py` now asks agentm for the project's `designs/` through `scripts/project_homes.py`. Its verbs are `designs-home`, `design-path <slug>` (a confidential design, `<designs>/<slug>.md`) and `parts-dir <slug>` (`<designs>/<slug>/parts/`, for a published design too, since parts are working documents), and `harness-root` is gone. With no designs home it exits 3 and writes nothing. A published design still authors standalone. `/spec` writes `<desk>/briefs/<slug>-spec.md`, the skeleton's place for a brief with no task yet. The module also stops loading development-lifecycle's `resolve_plan.py` at import time. *Why not keep parts beside the doc:* a published doc sits in the repo's `wiki/designs/`, and parts are the project's working records, which agentm-vault's own parts already keep in the vault. *Re-audit* if a repo without agentm needs translate.

**2026-09-13 — the voice inputs name the memory-root trims' homes.** agentm-vault plan 05 (2026-09-11) folded `voice-kernel.md` into the `## Voice` section of `<vault>/standards/user-preferences.md` and moved the cross-project voice rules from `_global/wiki-style/` to `<vault>/standards/voice/`. References now names those homes first, and the retired paths second as the fallback `prose_pass.py` and `vault_layout.py` still probe. *Why not drop the retired paths:* a vault that never migrated still resolves through them. *Re-audit trigger:* the fallback rungs leave `prose_pass.py` and `vault_layout.py`.

**2026-07-16 — the prose pass becomes the default for authoring, not just a standalone tool (operator escalation of the same-day session rule).** `/design author` now runs the `prose-pass` skill (below) over the full document at Step 5 (author signals ready) and on review-pass entry, automatically — the operator no longer invokes it by hand. Claude generates the fact-guard list *before* calling `prose_pass.py`, using the default `design-doc-prose` overlay; the pass folds into that day's consolidated Amendment-log entry rather than adding a separate row. Degradation (exit 1/2) is graceful AND announced: the caller relays the `PROSE-PASS-DEGRADED` stdout marker, runs a Claude-only conformance pass instead, and continues — never blocks authoring. Same wiring lands in the `wiki` plugin's `documenter` agent (sibling-plugin resolution — design stays the sole owner of the primitive; wiki does not gain a hard `requires:`). *Host note (forward reference, named not built):* on Antigravity the host model is already Gemini, so the cross-model roles invert — the simplifier there would be Claude; deferred until the AG host has a Claude transport worth wiring. *Re-audit trigger:* `agy` retiring or its roster dropping the "Gemini 3.1 Pro (High)" pin; or building the AG role-inversion.

**2026-07-16 — headless cross-model prose pass shipped (`prose-pass`, design plugin 0.7.0).** The "external (cross-model) review" line item split in two, and the headless half shipped: `scripts/prose_pass.py` (Gemini simplifies via `agy`, mirroring cross-review.sh's prompt-first invocation and exit 0/1/2 degradation contract; a mandatory caller-supplied FACT-GUARD list plus the operator's voice pack inlined verbatim from the vault; frontmatter / headings / table structures / Document History validated byte-identical with one retry) + `skills/prose-pass/SKILL.md` (the Claude verification checklist, then deploy + one consolidated Document History row per day). Pattern proven by hand 2026-07-16 against agentm's capture design, where the un-guarded first pass inverted document truths and asserted a staging path the design's whole point is to bypass — hence the fact-guard block and the verify step being non-optional. *Why not wait for #5b:* the interactive handoff (transfer-context + resume session) is host-specific plumbing; the headless pass is host-agnostic and already proven — shipping it first captures the value without blocking on the session-handoff design. #5b stays deferred and will share the fact-guard + voice-pack discipline. *Re-audit trigger:* when #5b ships, fold both under one external-review umbrella; if `agy`'s invocation contract changes (prompt-first, stdin-closed), re-audit `prose_pass.py` alongside cross-review.sh.

**2026-07-07 — `how-we-engineer` wired as a cross-plugin interpolated binding; reconciled "implements" → "consumes" wording (PLAN-wave-d-personas task 2).** Added `how-we-engineer` to `design.md`'s `opinions:` frontmatter alongside `good`, and wrapped the rung-picking prose (the "Rungs" table's sizing call) in an `<!-- opinion:how-we-engineer -->` marker — `generate.py build` splices the committed snapshot's body into it, proven byte-identical across two consecutive builds and both hosts. This doc previously said design **implements** `how-we-engineer`; corrected throughout to **consumes** — per the opinion's own `implements: crickets/development-lifecycle` pointer, design is a genuine cross-plugin consumer (the same class as its already-wired `good` binding), not the implementer. *Re-audit trigger:* if a future change moves `how-we-engineer`'s `implements:` target, re-check this doc's consumer/implementer framing against it.
