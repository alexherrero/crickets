...
The battery must be deterministic to be trustworthy: gate scripts are exit-code-honest, the generator's determinism keeps the drift gate from flapping, and a flaky gate is treated as a bug, not retried into green. That discipline got a live proof on 2026-06-09: the first Windows `validate` run failed the drift gate because `Path.write_text()` newline-translates on Windows — the generator wasn't byte-deterministic *across OSes*. The fix is structural (all generated files are written as bytes, `write_utf8`), not a retry. The aggregate workflow makes partial signal (one OS green) impossible to mistake for full green.
...
| 2026-06-09 | **The extension caught its first bug on its first run:** Windows' new `validate` job failed the drift gate — `Path.write_text()` newline-translates `\n`→`\r\n` on Windows, so the generator's JSON output wasn't byte-identical to committed `dist/`. Fixed structurally: all generated files now write bytes (`write_utf8` in `generate.py`, all 11 emit sites converted). Recorded under Reliability as the live proof of the determinism discipline. | draft |
