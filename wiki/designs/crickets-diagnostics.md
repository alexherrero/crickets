...
**`[PENDING-IMPL]`** — **V6-11's substrate shipped 2026-07-06** (agentm's [memory index](https://github.com/alexherrero/agentm/wiki/agentm-memory-index) design: the `entry_meta` metadata table now carries the indexed `fingerprint` column, and `failure-incident` is a reserved `kind` with a mandatory privacy scrub already wired on the write path). Layer-1's substrate is no longer the blocker — **still to build:** the fingerprint normalizer, the recall ladder itself (Layer 0 classify → Layer 1 exact-match → Layer 2 semantic fallback), and `/diagnose`. Documenter flips this marker when those land.
...
On-demand `/diagnose`: classify → recall (fingerprint-first) → rank hypotheses → log a `failure-incident` — **V6-11 shipped 2026-07-06**, so Layer-1's substrate (the indexed `fingerprint` + `project` columns) is ready; `/diagnose` itself still needs building. Scheduled health passes are no longer blocked on an unbuilt scheduler either (**the agentm runner shipped the same day**) — they wait only on `/diagnose` existing to have something to schedule.
...
- **Layer-1 must be project-scoped.** `SELECT WHERE fingerprint = :fp` **collides across projects** if the fingerprint doesn't carry the repo (same error class + frame in two repos → the wrong incident). **The V6-11 metadata table now carries its own `project` column** (derived from the entry's `group:` frontmatter) alongside `fingerprint` — Layer-1 can filter on both in one query (`WHERE fingerprint = :fp AND project = :proj`) rather than folding project into the fingerprint string itself.
- **The greenfield is real** — the diagnose engine + the recall ladder are the heaviest build of the new capabilities.
- **V6-11 shipped 2026-07-06** — the indexed `fingerprint` (and `project`) columns Layer-1 needs now exist in the real `entry_meta` table (agentm's [memory index](https://github.com/alexherrero/agentm/wiki/agentm-memory-index) design); no grep-over-frontmatter interim needed. What's left is diagnostics' own build: the normalizer, the ladder, `/diagnose`.
