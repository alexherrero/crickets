...
**2026-09-13 — the bridge runs agentm's code with agentm's own sibling modules (diagnostics 0.1.7).** agentm's scripts bare-import their siblings, at module level and inside functions, and a bare import returns whatever `sys.modules` already holds under that name. crickets ships modules under some of the same names (this plugin's `fingerprint.py`, the wiki plugin's `vault_layout.py`), so a shared process such as the unit suite could hand one to agentm: `recall.query` imports `lifecycle` inside the call, `lifecycle` imports `vault_layout`, and a pinned `save_entry()` imports `vault_layout` too. Every load and every call in `agentm_bridge.py` (`query_semantic`, `write_failure_incident`, `opinion_resolve`) now runs inside `_agentm_names`, which puts agentm's scripts dir first on `sys.path` and sets aside same-named modules loaded from other files, then puts them back. `scripts/test_diagnostics_agentm_bridge.py` pins each call against a stand-in agentm. `diagnose.py` already loaded its own `fingerprint.py` under a private name it never registers, so a `/diagnose` run was not affected. *Why not rename the crickets modules:* agentm's script list keeps growing, and a rename covers only the names that clash today. *Re-audit trigger:* a new bridge function that calls agentm without `_call`.
