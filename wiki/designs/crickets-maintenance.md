...
**2026-09-13 — debt and content-refresh entries file under `memory`, not the retired `personal` (maintenance 0.2.8).** `agentm_bridge.py`'s two writers defaulted to `group="personal"`, and agentm files a note under the group it is handed, so every `debt` and `content-refresh-watchlist` entry landed in `personal/`, the memory space's name before stage 2. Both now default to agentm's own default group, `memory`. Entries already written under `personal/` are not moved. The bridge also runs every load of and call into `save.py` inside `_agentm_names`, as the research and diagnostics bridges do, because `save_entry()` imports `fingerprint` inside the call when no fingerprint is passed and the diagnostics plugin ships a `fingerprint.py`. *Why not omit the group and let agentm choose:* the explicit default keeps the destination visible where the entry is written. *Re-audit trigger:* agentm changing its default group, or retiring `memory/` as one.
