...
- **ci** *(partial)* — `check-all.sh` is the single source of truth for "green"; the rules: always set up CI on a new project · run the battery before every commit · add a new check to the battery as the project grows. The **both-places consistency is mechanically enforced** — `scripts/test_ci_consistency.py` (itself a battery test) fails if a battery gate is missing from the Linux workflow or a toolchain gate from any of the three OS workflows, and the aggregate `ci-all.yml` couples to the three OS workflows by filename (a rename trips the test). The gate inventory is a `reference/` doc. **A push whose full diff is confined to `wiki/**` and/or `*.md` files is docs-only and skips the 3-OS test matrix + `ci-all`** — generalizing the existing `wiki-sync.yml` `paths: [wiki/**]` precedent via a `paths-ignore` filter on the test/aggregate workflows; the classification is diff-based and conservative — any file outside that pair counts as code, so a diff mixing one code file with any number of doc files still runs the full matrix, no exceptions. [development-lifecycle](crickets-development-lifecycle.md)'s pipelined wake-on-CI cadence governs *when* `/work` consults this gate; this domain owns the gate rule itself. Consumed by every pre-commit and by `maintenance`'s CI-repair.
