...
| **agentic-engineering** | phase-gated sessions · state-on-disk · single-threaded + read-only fan-out · the PLAN.md shape · wake-on-CI · no-parallel-implementers · **the single-cycle shape** (cooldown-gated · cursor-backed · idempotent · opt-in · surface-don't-adopt) | partial *(mostly homeless today)* |
...
- **ci** *(amended — per-plan CI)* — `check-all.sh` remains the single source of truth for "green"; the rules: always set up CI on a new project · run the battery before every commit · add a new check to the battery as the project grows. The **both-places consistency is mechanically enforced** — `scripts/test_ci_consistency.py` (itself a battery test) fails if a battery gate is missing from the Linux workflow or a toolchain gate from any of the three OS workflows, and the aggregate `ci-all.yml` couples to the three OS workflows by filename (a rename trips the test). The gate inventory is a `reference/` doc. **The full 3-OS matrix + `ci-all` now fire on `pull_request`, not on push to `main`.** A push directly to `main` (a merge landing) runs, at most, a light lint/typecheck smoke job — not the full matrix; the PR that produced the merge already ran the full matrix as a required check before GitHub allowed the merge, so re-running it post-merge is redundant work, not redundant safety. Branch protection (ADR 0029, unchanged) requires the `check-all.sh`-backed PR check green before merge is even offered. **The docs/wiki-only diet does not carry forward onto the `pull_request` trigger as-is** — a trigger-level `paths-ignore` would mean a docs-only PR's required check never fires, and a required check that never reports blocks that merge forever (demoting it to non-required would let code PRs skip it instead). Every PR gets the full matrix for now; the diet's docs-only fast path re-lands as a **job-level** no-op inside `ci-all` (diff-aware, not trigger-aware) under `PLAN-per-plan-ci` — filtering moves from "does this trigger fire" to "does this job actually run once it has." [development-lifecycle](crickets-development-lifecycle.md)'s wake-on-CI cadence becomes **wake-on-PR-checks**: `/work` waits on the PR's check-suite instead of a bare push's; the pipelined one-task-lag cadence (2026-07-05 amendment) still governs *when* it's consulted, unchanged in shape. `wiki-sync.yml` is unaffected — it was never part of the test matrix and stays push-triggered. **Nightly main-HEAD backstop** (lands with `PLAN-per-plan-ci`): a scheduled workflow reruns the full matrix against `main` HEAD independent of any PR, advisory-only — the drift-between-merges catch that push-triggered CI used to provide for free; `agentm`'s existing `health-nightly.yml` (a different, heavier functional-quality tier) gets this wired into its scorecard render, `crickets` gets the workflow fresh. Consumed by every pre-commit and by `maintenance`'s CI-repair.
...
- **agentic-engineering** *(partial — mostly homeless)* — the harness discipline: phase-gated sessions, state-on-disk-not-conversation, single-threaded implementation with read-only sub-agent fan-out, the PLAN.md shape (locked design calls + narrative Status), wake-on-CI, no parallel implementers. Today these live only in `AGENTS.md` / `principles.md` / `CLAUDE.md`; they migrate in. Consumed by every phase workflow and every persona.
  - **The single-cycle shape (named 2026-07-06).** A background primitive built to run **often and cheaply when there's nothing to do**: **cooldown-gated** (a cursor + a per-item marker decide what's due, so a repeat call this soon is a cheap no-op); **cursor-backed** (progress lives in a local, non-synced marker — never in memory, never re-derived); **idempotent** (a crashed or re-fired cycle re-runs whole safely, because the cursor and the dispatched-set make repetition harmless); **opt-in** (nothing runs until an operator or a job manifest turns it on); **surface, don't adopt** (the primitive reports what it did; a caller decides what to do with the report, the primitive never acts unilaterally beyond its own scope). Two independent implementations share this shape: `wiki-watch`'s one-cycle drift-detection pass (crickets), and the [agentm runner](https://github.com/alexherrero/agentm/wiki/agentm-runner)'s job-manifest cycle (agentm, built 2026-07-06) — named here so the two read as one documented pattern, not two coincidentally-similar mechanisms invented twice.
...
- **documentation** *(partial)* — the **Diátaxis structure**: every page is exactly one of four modes (tutorials · how-to · reference · explanation), single-mode-per-page, with length ceilings and a naming style. The tree is the **fixed six-section frame** in fixed order — How-to · Reference · Architecture · Designs · Explanation · Operational — with two **conditional** sections (Architecture on a `wiki/architecture.yml` declaration; Operational on audience/visibility). The **naming rule:** every page basename is unique across the tree case-insensitively; when a user-facing page and a design page collide, the user-facing page owns the clean name and the design page takes a `-design` suffix (`check-wiki` rule (g)). Index/landing pages (`<!-- mode: index -->`) are **shape-exempt** from the single-mode rule. The objective structure standard lives here; **[wiki](crickets-wiki.md) consumes and enforces it** — `check-wiki.py` is the gate (mode · length · vendor · structure), its templates and `diataxis-author` skill author against it, and a per-repo `.diataxis-conventions.md` can override. The prose *voice* is a separate agentm opinion (subjective, learned), not a convention.
...
**2026-07-23 — releasing domain: ship-release gains an HLD-update close-out check (Loose Ends arc, "Release and generator polish" task 1); this doc's own `governs:` glob corrected.** Standing gap named by a FOLLOWUPS entry filed at Plan #18 (2026-05-26, still open as of 2026-07-14): a release could change architecture — new/removed primitive kind, a generator-behavior change, anything a living design should reflect — and ship without that design ever being checked, and nothing caught it. `ship-release/SKILL.md` gains pre-release checklist item 9; `development-lifecycle`'s `/release` command gains the matching constraint 10 + Process step 4c (Hook 7 in that design's own hook-numbering sequence). Both key off the plan's *existing* `touches_architecture:` self-report (already wired since Hook 3's `/plan`-time grounding gate) rather than inventing a new architecture-changed heuristic — the new step fires on the flag alone, catching what the existing `parent_design_doc` divergence check (Hook 5, constraint 4b) structurally can't: architecture changed and no design was ever cited at all — 4b only ever catches a design that *was* cited and then drifted from a locked call. *Why not fold this into `coalescence-gate`'s seven-item checklist as an eighth item:* that checklist is deliberately arc-closing-conditional (its own "What is NOT an acceptable shortcut" table treats the set as fixed, and it only fires when a release closes an arc); this check needs to fire on *every* `touches_architecture: true` release, not just arc-closing ones, so it lives in `release.md`'s unconditional Process instead — `coalescence-gate.md` gets a one-line See-also cross-reference, not a member of the seven. *Why not CONTRIBUTING.md* (the FOLLOWUPS entry's third checked-and-found-absent location): that file is contributor-onboarding scoped (authoring a plugin, testing locally, commit messages) with no release section today — the mechanism belongs in the files that actually drive a release. **Aside, same landing:** this design's own `governs:` frontmatter glob was found stale mid-plan — still naming the pre-merge `src/releasing-conventions/`/`src/testing-conventions/` directories from before the AG Wave A merge into `src/conventions/` (2026-06-23, this log's own bottom entry), so `find_governing_design.py` was silently falling through to the parent HLD's broad `src/**` fallback for anything under `src/conventions/`. Corrected to `governs: [src/conventions/]`. *Re-audit trigger:* if a mechanical "did this release change architecture" detector is ever built (replacing the plan-author's `touches_architecture:` self-report with something that inspects the diff directly), revisit whether the self-report step here is still needed or can retire in its favor.

**2026-07-06 — ci domain: per-plan CI, the matrix moves from push to pull_request (operator-ratified 2026-07-06; ROADMAP-MASTER § Ratified 2026-07-06; adjudicated via the W1 verdict memo + Fable judgment pass; PLAN-worktree-native-flow / PLAN-per-plan-ci).** The full 3-OS matrix + `ci-all` now trigger on `pull_request`; a push to `main` runs at most a light lint job. Branch protection requires the PR-triggered `ci-all` check green before merge. The docs/wiki-only `paths-ignore` diet is intentionally **not** carried onto the `pull_request` trigger in this wave — a trigger-level skip on a required check means a docs-only PR's check never reports, which blocks that merge forever (a bug the Fable judgment pass caught before it shipped). Every PR runs the full matrix for now; the diet returns as a job-level, diff-aware no-op inside `ci-all` under the follow-on `PLAN-per-plan-ci`. Wake-on-CI becomes wake-on-PR-checks. `wiki-sync.yml` is unaffected (rider 4, Fable pass) — it was never the test matrix. A nightly main-HEAD full-matrix backstop (advisory-only) lands with `PLAN-per-plan-ci`, closing the gap that push-triggered CI used to cover for free. *Why not carry the paths-ignore diet forward unchanged (the original draft's assumption):* GitHub can't express a required check that sometimes doesn't run — the trigger-level filter and the required-check contract are mutually exclusive until the filter moves to job level. *Re-audit triggers:* the job-level diet ships in `PLAN-per-plan-ci` (this bullet's "every PR runs the full matrix for now" language should then be corrected); GitHub ships a native way to make a trigger-filtered check merge-transparent.

**2026-07-06 — named the single-cycle shape in `agentic-engineering` (AG Wave B leader 1/5, the agentm runner's own Migrations bullet).** `wiki-watch`'s one-cycle drift-detection pass and the newly-built [agentm runner](https://github.com/alexherrero/agentm/wiki/agentm-runner)'s job-manifest cycle independently converged on the same shape (cooldown-gated · cursor-backed · idempotent · opt-in · surface-don't-adopt) without either citing the other. Named it here so future background primitives have one documented pattern to point at instead of re-deriving it. *Why not leave it un-named:* two coincidentally-similar mechanisms invented twice is exactly the homelessness this domain exists to fix. *Re-audit trigger:* a third independent implementation surfacing a shape variant this description doesn't cover.

**2026-07-05 — ci-battery path-filter: docs/wiki-only diffs skip the 3-OS matrix + `ci-all` (PLAN-ci-walltime-diet task 3).** Generalizes the existing `wiki-sync.yml` `paths: [wiki/**]` precedent (previously scoped only to that one workflow) into a named rule for `tests-{linux,mac,windows}.yml` + `ci-all.yml`: a push confined to `wiki/**` + `*.md` is docs-only and self-skips the matrix. Evidence (2026-07-03..05, agentm+crickets): 9-32% of pushes in-window touched zero code paths yet still ran the full matrix — 60 wasted workflow runs across both repos in a single 2-day window. The guard is deliberately conservative: any file outside `wiki/**`/`*.md` counts as code, full stop — a diff mixing one code file with 500 doc files still runs the full matrix. Cites [development-lifecycle](crickets-development-lifecycle.md)'s 2026-07-05 pipelined wake-on-CI cadence amendment for *when* `/work` consults this gate; this entry only owns the gate rule itself, per the cite-don't-duplicate boundary this domain already states about itself.

*Why not a broader doc-extension allowlist:* the conservative two-pattern rule already covers the evidence window's docs-only share; a broader allowlist (e.g. other markup/config formats) raises the guard's surface for a fixture to prove exhaustively, for a savings the evidence doesn't currently justify.

*Re-audit triggers:* a future language/format is added under `wiki/**` that isn't itself documentation (e.g. a generated script checked into `wiki/`) — revisit the glob; or the docs-only share of pushes grows enough that a broader allowlist's savings would clearly outweigh its guard-surface cost.

**2026-06-30 — Decisions section retired: six-section taxonomy (the ADR-0020 re-audit trigger fired).** With the ADR model retired and decisions living in design amendment logs, the `decisions/` section is universally empty — the exact "drop it" case ADR-0020 (below) named as its re-audit trigger. The `documentation` domain body now reads a **fixed six-section frame** (How-to · Reference · Architecture · Designs · Explanation · Operational; Architecture + Operational conditional, the other four always present). `wiki_init.py`'s `DEFAULT_SECTIONS`, `check-wiki`'s `_FOLDER_MODE` + the accepted-ADR rule (f), the `documenter` / `migrate.py` ADR-routing, and the project templates drop `decisions/` in the same pass; agentm + crickets delete their (already-empty) `wiki/decisions/` tombstones. *Why not keep the slot:* a permanently-empty always-present section is drift that re-scaffolds itself. *Re-audit trigger:* a durable decision-artifact class with no living-design home (re-add a section); a host that reintroduces the ADR model as the default.
