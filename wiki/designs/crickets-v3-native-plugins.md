...
- **`enhances:` groups** (`developer-safety`, `code-review`, `wiki-maintenance`) are standalone too, but soft-augment `developer-workflows` at its phase boundaries when both are installed (capability-probed; graceful-skip otherwise) — #42's "integrated" mode, generalized by [ADR 0017](crickets-composition).
