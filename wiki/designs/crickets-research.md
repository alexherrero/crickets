...
- *Artifacts:* `learn-forward` surfaces via agentm's own forward-learning output path; `codebase-improvement` writes exactly one watchlist finding per stale pattern found, in the same entry shape `forward_learning.py` uses (`<watchlist>/codebase-improvement/<slug>.md`, where `<watchlist>` is `Projects/agentm/_watchlist/` since agentm's memory-root trims and the memory-space spelling on a vault that has not moved; `status: pending-review` frontmatter), so agentm's `watchlist_review.py` picks it up as part of the same merged review surface — **surfaced, never adopted silently**.
...
**2026-09-13 — call-time imports in the real-bridge suites get agentm's siblings too.** The entry below left one re-audit trigger open, a bare sibling import that agentm makes inside a function, and it had already fired through a path that entry does not name. `recall.py` imports `lifecycle` inside a function, and lifecycle's own `import vault_layout` ran after the swap had put the suite's modules back, so four real-bridge suites, idea-search's among them, handed agentm the wiki's module and passed only because none asserts on lifecycle's sidecar. `scripts/agentm_isolation.py` now sets same-named modules aside for the life of a real-bridge test class, with agentm's scripts dir first on `sys.path`, and every real-bridge suite except learn-forward's calls it. Learn-forward's stays on the bridge's swap, so its tests keep exercising that swap end to end. The bridge is unchanged. *Why not in the bridge:* a swap that outlived the load would hand the process's own code agentm's modules, and the research CLIs, each in a process of its own, never meet the clash. *Re-audit trigger:* a research CLI running in a process that also loads the wiki plugin's scripts, or the import hook finding a borrow again.

**2026-09-13 — the bridge gives agentm its own siblings, and the learn-forward test names the watchlist's homes again (research 0.2.4).** The entry below set same-named modules aside in the test, which protected one test class. The clash sits in the bridge: any process that already holds another `vault_layout` hands it to agentm. For the length of a load, `agentm_bridge.py`'s `_load_module` now puts agentm's scripts dir first on `sys.path` and sets aside any module held under one of that dir's script names but loaded from another file, then puts those modules back. The test's own set-aside is gone, so the real-bridge tests go through the bridge's; `scripts/test_research_agentm_bridge.py` pins it against a stand-in agentm, so CI runs it. The no-writes test names the watchlist's homes again, `Projects/agentm/_watchlist` plus the memory-space spellings, and `setUp` writes the whitelist at `SOURCES_CONFIG_REL`. `watchlist_root()` and `sources_config_path()` only exist since agentm #602, so asking them made the test error against any older agentm. The content snapshot and the wrote-nothing guard stay. *Why not keep asking agentm:* an older agentm checkout could not run the test at all; the cost is that on the current agentm the test also accepts the retired homes. *Re-audit trigger:* the watchlist moving again, or a bridge caller reaching a bare sibling import that agentm makes inside a function. The swap covers module execution only, so such an import gets the restored module; `recall.py`'s `session_start` and `_collect_always_load_paths` already import `vault_layout` that way.

**2026-09-13 — the learn-forward test asks agentm's resolvers where the watchlist is.** agentm's memory-root trims moved the watchlist to `Projects/agentm/_watchlist/` and put the sources whitelist beside it. `scripts/test_research_learn_forward.py` still allowed only memory-space watchlist paths, so it failed a correct scan. In the full suite it errored before reaching that check: the wiki plugin's `vault_layout` module, loaded earlier in the same process, shadowed the `vault_layout` that agentm's `forward_learning.py` now imports. The test now takes both homes from agentm's `watchlist_root()` and `sources_config_path()`, sets same-named modules aside while agentm is loaded and restores them afterwards, and stubs the fetcher in its CLI smoke test, which had been sending a real GET. The body names the resolvers where it named `personal/_watchlist/` and `_meta/`. *Why not list the new paths:* the watchlist has moved three times, and a list of paths went stale on each move. *Re-audit trigger:* agentm renames either resolver, or CI starts checking out agentm for these tests.
