...
**`wiki-maintenance`** is a native crickets plugin (`requires: [developer-workflows]`) that bundles the wiki toolchain — the **`wiki-author`** skill (the authoring + learning engine), the **`documenter`** agent (phase-boundary structural sweeps), the `diataxis-evaluator`, the `recent-wiki-changes` helper, and the `check-wiki.py` linter. It composes with `developer-workflows` via **`enhances:`** soft-composition ([ADR 0017](decisions/0017-enhances-soft-composition.md)): the phase commands already scaffold a `documenter` dispatch at every boundary, and this plugin is what that dispatch resolves to.
...
**Durable storage**
- **The convention/overlay store** — read **on-demand** via the resolver, deliberately **outside `_always-load`** so it doesn't weigh down every session's context. Three scopes: **global** (on-demand, reserved `_global` slug), **per-project** (`projects/<slug>/wiki-style/`), **per-repo** (`wiki/.diataxis-conventions.md`). Scope follows the harness storage config (repo / device-user / central vault). *Future: when agentm V6 indexed-recall ships, the global tier moves from file-read to vector-discovery — see the V6 re-audit in [`agent-memory-evolution.md`](agent-memory-evolution.md).*
...
*(these become the `/design sequence` parts)*
...
- Ships **granularly across crickets 3.x minors**, parts in dependency order (1→2→3 core, 4 watcher, 5 dogfood). Plugin install *is* the feature flag — no separate flag.
...
| Date | Change | Status |
|---|---|---|
| 2026-06-04 | Initial draft created via `/design author`. | draft |
| 2026-06-04 | Full draft authored via `/design author` walk — all 10 sections. Identity pivot (Diátaxis as floor, not law → template-driven + operator-voice learning loop); group renamed `wiki`→`wiki-maintenance`; `standalone:true`+`enhances` composition; the wiki-watcher continuous mode (§7, W1-now/W3-later); bucket-A fold-in vs bucket-B kernel-graceful-skip split; `_always-load`→on-demand convention storage. Cross-doc notes placed in `memory-os-architecture` (V5 memory-plugin re-audit) + `agent-memory-evolution` (V6 always-load cutover). | draft |
| 2026-06-04 | Author signaled ready for review. | review |
| 2026-06-04 | Approved as final via `/design author` (inline approval — design collaborated section-by-section during the authoring walk, so no separate review pass needed). Unblocks `/design translate`. | final |
| 2026-06-04 | Completed Detailed Design §7 (the wiki-watcher) — the one subsection left stubbed at finalization — with the agreed W1-now/W3-later, PR-default, config-rides-`.agentm-config.json`+per-repo-marker+`repo_registry`, cursors/processed-set, significance gate, audit-log design. Operator-directed completion at translate time; no other section changed. | final |
| 2026-06-04 | Translated to 5 parts via `/design translate`: scaffold-fold-in, style-learning-loop, documenter-wiring, wiki-watcher, dogfood-finale. | final |
| 2026-06-04 | Sequenced into 5 plans via `/design sequence`; first plan active at the vault `_harness/PLAN.md` (scaffold-fold-in), 4 queued at `_harness/designs/wiki-maintenance/queued-plans/`. Topological order (Kahn, alphabetical tie-break): scaffold-fold-in → documenter-wiring → style-learning-loop → wiki-watcher → dogfood-finale. Strengthened `dogfood-finale`'s dependency to include `wiki-watcher` so the finale (whole-plugin validation, gates the V5 slim) sorts last. State written to the vault `_harness/` (this project redirects harness state to the Obsidian vault, not local `.harness/`). | final |
