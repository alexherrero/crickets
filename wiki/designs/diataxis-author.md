...
**Read** (on every skill invocation): glob `<vault>/personal-private/_always-load/diataxis-*.md` → load operator conventions (filename style / mode-mixed split preferences / template overrides / project-specific exceptions). Falls back to ADR 0004's defaults if no AgentMemory entries exist. *(Since superseded: the wiki-maintenance plugin's part 3 relocated the global conventions out of `_always-load` into the on-demand `_global` store, read via the `documenter-context` resolver — see the [wiki-maintenance design](wiki-maintenance-design).)*
...
No feature flags; no phased rollout (single operator, no user segments). Escape hatches: `diataxis-author.enabled: false` skill-level config to disable; uninstall to remove entirely (originally `bash crickets/install.sh --uninstall diataxis-author`; since v3.0 the skill ships inside the `wiki-maintenance` plugin, so removal is `claude plugin uninstall wiki-maintenance`); per-repo `wiki/.diataxis-conventions.md` for project-specific overrides without touching global state.
...
2. **Skill uninstall**: originally `bash crickets/install.sh --uninstall diataxis-author`; since v3.0 the skill ships inside the `wiki-maintenance` plugin, so removal is `claude plugin uninstall wiki-maintenance` (`agy` equivalent on Antigravity). Per-repo `.diataxis-conventions.md` files + the operator's vault conventions are operator-owned data, untouched.
...
| Date | Change | Status |
|---|---|---|
| 2026-05-22 | The full lifecycle in one day. Drafted via `/design author` (the 4 load-bearing questions answered, all at the recommended defaults: scope = author+maintain+migrate, subsuming the `migrate-to-diataxis` predecessor; AgentMemory read **and** write with per-repo overrides; `documenter` kept as the dispatch worker; single multi-sub-command skill on the `/memory` shape); all 10 sections + 11 QA sub-attrs filled; approved as final (fast-path); translated + sequenced into 5 parts (skill-scaffold → author-classify → check-repair → migrate-subsume → agentmemory-docs-release) — then **all 5 parts executed + the paired release shipped** (crickets v0.11.0 substantive + agentm v2.4.3 paired-doc-only). **3 Windows-specific CI failures caught + fixed mid-plan** (Start-Process arg split; `git mv` cwd dependence; cp1252 stdout encoding). Predecessor deprecated (removal in a follow-up harness PATCH). Status `final → launched`. | launched |
| 2026-06-09 | **Post-launch evolution recorded** (content review; structure unchanged). The skill was repackaged twice after launch: crickets **v3.0** retired the `install.sh` copy-install (the skill now generates as part of a native plugin), and the **wiki-maintenance plugin** (bucket ④ — [design](wiki-maintenance-design)) folded it in as the authoring engine, pivoting its contract from "enforce Diátaxis modes" to "Diátaxis as floor + operator templates/voice," adding edit-driven voice capture + the `style-scope-evaluator`, and relocating the global conventions from `_always-load` to the on-demand `_global` store. This doc stays the canonical record of the skill's internals as designed; the wiki-maintenance design records the evolution. | launched |
