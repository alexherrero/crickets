...
**2026-06-28 — lock-down sweep (operator review).** Sized the capability-example diagram (`width`/`height` so it renders large). The amendment log was already newest-first and the body cites no retired ADR numbers (the ADR mentions are legitimate fold records), so nothing else changed. *Locked as a v5–v8 guidepost.*
...
**2026-06-24 — reconciled to the now-final child designs.** All 14 AG child designs are content-final, so the body is brought current:

- the worked example + caption use `development-lifecycle` (the renamed spine);
- the capabilities note completes the rename ledger;
- the capability rows name the facts that landed in review — `github-projects` (≥4 depth floor · per-commit comment trail · driven by the **Planner (TPM)** persona), `maintenance` (+ the tentative `content-refresh` primitive), `conventions` (the 8-domain base-standards shell, incl. the new **documentation** domain that owns the Diátaxis structure `wiki` enforces);
- the "per-capability sub-designs not yet written" caveat is closed;
- the new cross-cutting **[model + effort routing](https://github.com/alexherrero/agentm/wiki/agentm-model-effort-routing)** design (the model × effort tier scale + the `tier:` persona-manifest axis — agentm-parented) is named in Sub-designs.
