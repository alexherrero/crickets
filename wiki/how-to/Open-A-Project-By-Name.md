...
> **Prereqs:** the `development-lifecycle` plugin installed ([Install crickets plugins](Install-Crickets-Plugins)). Works standalone; the vault-projects and recall sources degrade gracefully when no agentm clone is mounted (see [Development Lifecycle](Development-Lifecycle)).
