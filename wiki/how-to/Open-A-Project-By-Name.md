...
> **Prereqs:** the `development-lifecycle` plugin installed ([Install crickets plugins](Install-Crickets-Plugins)). Without agentm the command still finds a project, but shows no plans: development-lifecycle reads them through agentm (see [Development Lifecycle](Development-Lifecycle)).
...
3. **Read the orientation.** Once you confirm, the command prints the rendered block as it is. It leaves out any section that has no source:
   ...
   - **Plans** — every plan agentm lists: by the project's repo checkout, or by its name when it has none. The ones in flight come first, ordered by their tracker's importance, each with its status and step checklist. Finished plans show as a count. When agentm lists none, one line says so.
   - **Recent progress** — the last few progress lines of each unfinished plan.
   - **Queued plans** — tasks whose tracker is `queued`.
   - **Board state** — the project's rows in the board ledger github-projects reads.

4. **Optionally, keep the orientation as a note.** Add `--note` to write the same block to `orientation-note.md` in the project's `desk/`, replacing the last one. The desk is the one agentm names for the project; when there is none, the command writes nothing and tells you so.

## Verify

- The command asks you to confirm before it renders anything.
- Nothing changes on disk (`git status` in the project shows nothing new) unless you passed `--note`, which writes only the desk's `orientation-note.md`.
...
- [Named plans](Named-Plans) — how plans and their trackers are named and resolved.
