...
- [Developer safety design — worktrees first-class but operator-initiated](crickets-developer-safety) — the norm that sanctions the worker worktrees this command merges and prunes.
- [Development lifecycle design — gate the integrated tree](crickets-development-lifecycle) — *why* the gate runs on the merged tree and `main` hard-resets on red rather than gating the worker branch in isolation.
