...
- **A phase command didn't emit a board update**: emission is gated on the plugin being installed — `/plan`, `/work`, `/release` probe `capability_probe.py github-projects` and graceful-skip when it returns non-zero. Confirm the plugin is installed and `project.json` + `gh` are present.
